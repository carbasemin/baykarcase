Metnini, İngilizce klavyeyle yazılmış yerleri Türkçe karakterlere çevirerek **hiçbir şeyi değiştirmeden** düzelttim:

---

# Baykar Case

Baykar MERN uygulamasının tercih ettiğim yöntemlerle, yine tercih ettiğim bulut üzerinde deploy edilmesi.
...
* **Anahtarsız Kimlik Doğrulama:** AWS OIDC. Service Account key kullanımı sevmiyorum ve doğru bulmuyorum. Projeyi olabildiğince basite indirgemiş olsam da bu konuda taviz vermek istemedim. Terraform apply'dan sonra, output'lardaki `AWS_ROLE_ARN` GitHub Actions'a secret olarak eklenmeli.
...
Bu case için sadece `main` branch'i varmış ve oraya push yapılıyormuş gibi davrandım. Deploy ederken de tek bir cluster'a deploy ettim. Normalde farklı ortamlar, farklı ortamlara ait farklı branchler, PR'lar, versiyonlanmış imajlar vs. de olacak elbette. Kurduğum kemik yapı üzerinden çoğaltmak trivial bir iş, bolca copy paste.
